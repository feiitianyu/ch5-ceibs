...
const homeInstanceModule = (id, elementIds) => {
    'use strict';    

    // BEGIN::CHANGEAREA - your initialization code for each instance of widget goes here  
    // console.log(`home-widget homeInstanceModule("${id}", [${elementIds}])`);
    ...
    const cleanup = () => {
        // console.log(`home-widget homeInstanceModule cleanup("${id}")`);
    };
...
const homeModule = (() => {
     ...
     */
    function onInit() {
       serviceModule.addEmulatorScenarioNoControlSystem("./app/project/components/widgets/home/home-emulator.json");
       // Uncomment the below line and comment the above to load the emulator all the time.
       // serviceModule.addEmulatorScenario("./app/project/components/widgets/home/home-emulator.json");
    }

    /**
     * private method for widget class creation
     */
    let loadedSubId = CrComLib.subscribeState('o', 'ch5-import-htmlsnippet:home-import-widget', (value) => {
        if (value['loaded']) {
            onInit();
            setTimeout(() => {
     ...
     */
    CrComLib.subscribeState('o', 'ch5-template:home-widget', (value) => {
        if (value['loaded'] !== undefined && value['id'] !== undefined) {
            if (value.loaded) {
                widgetInstances[value.id] = homeInstanceModule(value.id, value['elementIds']);
            }
            else {
